...
async function isNpmPackage(name) {
    try {
...
function installRepoWithArgsToClaudeDesktop(name, npmIfTrueElseUvx, args, env) {
    ...
    installToClaudeDesktop(serverName, npmIfTrueElseUvx ? "npx" : "uvx", [name, ...(args ?? [])], env);
}
async function attemptNodeInstall(directory) {
...
async function installLocalMcpServer(dirPath, args, env) {
    ...
    if (fs.existsSync(path.join(dirPath, "package.json"))) {
        ...
        Object.keys(servers).forEach((name) => {
            installToClaudeDesktop(name, "node", [servers[name], ...(args ?? [])], env);
        });
        return {
            content: [
                {
...
async function installRepoMcpServer(name, args, env) {
    if (!(await hasNodeJs())) {
        return {
            content: [
                {
